import { BlockTxBroadcastResult, LCDClient, MnemonicKey, Msg, MsgExecuteContract, Fee, Tx, TxBody, AuthInfo, Wallet } from "@terra-money/terra.js"
...
describe('operation', () => {
  it('generateWithAddress', async () => {
    ...
    const testLCDClient = new LCDClient({ URL: 'https://lcd.terra.dev', chainID: 'columbus-5' })
    ...
    testWallet.createAndSignTx = async ({ msgs }) => Promise.resolve(new Tx(new TxBody(msgs), new AuthInfo([], new Fee(1, "1uluna")), []))
