...
import { fabricatebAssetBond, fabricatebAssetBurn, fabricatebAssetClaimRewards, fabricatebAssetWithdrawUnbonded, fabricateTerraswapSwapbLuna } from "../../fabricators";
import { queryHubUnbond, queryRewardHolder, queryRewardState, UnbondResponse } from "../../queries";
...
export class BLuna {
  ...
  async getUnbondRequests(address: string): Promise<UnbondResponse> {
    return await queryHubUnbond({ lcd: this.lcd, address })(this.addressProvider)
  }

  async getClaimableRewards(address: string): Promise<string> {
    const holder = await queryRewardHolder({ lcd: this.lcd, address })(this.addressProvider)
    const rewardState = await queryRewardState({ lcd: this.lcd })(this.addressProvider)

    return new Int(
      new Int(holder.balance).mul(
        new Dec(rewardState.global_index).sub(new Dec(holder.index))
      )
    ).add(new Int(holder.pending_rewards)).toString()
  }
}